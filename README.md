...
Рассмотрим фрагменты кода, вычисляющие цвета пикселей. Первым идет код SIMD реализации, далее "наивной". Как видно, наивный алгоритм производит 3 сложения и 6 умножений над каждым пикселем. SIMD производит эти арифметические операции над 8 пикселями сразу, что позволяет значительно ускорить исполнение цикла.
...
С помощью сайта godbolt.org проанализируем различия при компиляции "наивной" и SIMD реализаций. Все программы компилировались с флагами -O2 и -mavx2. В случае с SIMD, компилятор полностью повторил наш код, обрабатывая по 8 пикселей за раз. В случае наивной реализации, компилятор не применил параллелизм, чем сильно замедлил работу программы.
