...
В наивной реализации компилятор обращается к памяти за каждым байтом, что сильно замедляет расчет цвета пикселя. Кроме того, наивный алгоритм производит 3 сложения и 6 умножений над каждым пикселем.
...
- Для того, чтобы уменьшить влияние факторов, не связанных с вычислением множества, каждый пиксель вычислялся по 10000 раз, после чего полученный FPS умножался на 10000.
